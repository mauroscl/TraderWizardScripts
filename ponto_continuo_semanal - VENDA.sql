...
pc21.codigo as pc21, pc21.percentual_volume_quantidade, pc21.percentual_candle2, pc21.percentual_volume_negocios, pc21.ValorMinimo, pc21.ValorMaximo, 
ROUND(pc21.MM21,2) MM21, pc21.Volatilidade, pc21.distancia
from			
...
from 
(
	select c.codigo, C.ValorMinimo, C.ValorMaximo, c.Negocios_Total, c.Titulos_Total, ((C.ValorFechamento - C.ValorMinimo) / (C.ValorMaximo - C.ValorMinimo)) as percentual_candle
...
inner join 
(	
	select c.codigo, ValorAbertura, ValorFechamento, ValorMinimo, ValorMaximo, M10.Valor, c.Negocios_Total, c.Titulos_Total, 
	...
	ROUND(ABS((c.ValorMinimo  * (1 - dbo.MaxValue(VD.Valor, MVD.Valor) * 1.25 / 100) / m21.Valor - 1)) * 100, 3) / 10 / dbo.MaxValue(VD.Valor, MVD.Valor) as distancia
...
WHERE 
--FECHOU ABAIXO DA MÉDIA OU ABAIXO DA MINIMA DO CANDLE ANTERIOR
(p2.ValorFechamento < p2.Valor OR P2.ValorFechamento < P1.ValorMinimo)
...
AND (
       ...
       AND (
			   ...
			   OR dbo.MaxValue(P2.percentual_volume_quantidade, P2.percentual_volume_negocios) >= @percentualDesejadoVolume
		   )
	   )
...
(
	...
	, M.Valor as MM21, c.Negocios_Total, c.Titulos_Total, 
	c.Titulos_Total / MVOL.Valor as percentual_volume_quantidade,
	C.Negocios_Total / MNS.Valor as percentual_volume_negocios,
	dbo.MaxValue(VD.Valor, MVD.Valor) as Volatilidade,
	((C.ValorFechamento - C.ValorMinimo) / (C.ValorMaximo - C.ValorMinimo)) as percentual_candle,
	ROUND(ABS((c.ValorMinimo  * (1 - dbo.MaxValue(VD.Valor, MVD.Valor) * 1.25 / 100) / m.Valor - 1)) * 100, 3) / 10 / dbo.MaxValue(VD.Valor, MVD.Valor) as distancia
	from Cotacao_Semanal c 
	inner join Media_Semanal m on c.Codigo = m.Codigo and c.Data = m.Data and m.Tipo = 'MMA' AND M.NumPeriodos = 21
	inner join Media_Semanal MVOL on c.Codigo = MVOL.Codigo and c.Data = MVOL.Data and MVOL.Tipo = 'VMA' AND MVOL.NumPeriodos = 21
	inner join MediaNegociosSemanal MNS on c.Codigo = MNS.Codigo and c.Data = MNS.Data
